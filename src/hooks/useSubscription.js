...
// trialing, provider: 'stripe' | 'asaas' }.
export function useSubscription(user) {
  ...
  const [resolved, setResolved] = useState(null); // { uid, status, plan, trialing, provider } | null

  useEffect(() => {
    let ignore = false;

    if (!user) {
      setResolved({ uid: null, status: 'inactive', plan: 'solo', trialing: false, provider: 'stripe' });
      return;
    }
    ...
    async function check() {
      const { data } = await supabase
        .from('profiles')
        .select('subscription_status, plan, payment_provider, access_until')
        .eq('id', user.id)
        .single();
      ...
      const ok = (data?.subscription_status === 'active' || trialing) && notExpired;
      setResolved({
        uid: user.id,
  ...
  if (!user) return { status: 'inactive', plan: 'solo', trialing: false, provider: 'stripe' };
  ...
  if (!resolved || resolved.uid !== user.id) return { status: 'loading', plan: 'solo', trialing: false, provider: 'stripe' };
  return { status: resolved.status, plan: resolved.plan, trialing: resolved.trialing, provider: resolved.provider };
}
