...
const tools = [
  ...
  },
  {
    type: 'function',
    function: {
      name: 'adicionar_gasto_debito',
      description:
        'Lança uma compra paga na hora — débito, Pix ou dinheiro. Não entra na fatura do cartão.',
      parameters: {
        type: 'object',
        properties: {
          nome: { type: 'string', description: 'Descrição do gasto (ex: "Mercado", "Uber").' },
          valor: { type: 'number', description: 'Valor em reais (ex: 49.90).' },
          categoria: {
            type: 'string',
            description:
...
function systemPrompt(context: {
  ...
  return [
    ...
    'e lance no lugar certo (assinatura, compra no cartão, gasto no débito, despesa fixa) seguindo as regras',
    ...
    'Comprovantes e notas quase sempre trazem a forma de pagamento impressa ("CRÉDITO", "DÉBITO",',
    '"PIX", "DINHEIRO"). Leia esse campo e use-o para escolher a lista, em vez de perguntar. Só',
    'pergunte se a imagem não disser.',
    '',
    '# COMO LANÇAR',
    ...
    'Há exatamente DUAS perguntas permitidas antes de lançar, e as duas são sobre informação que',
    'falta, nunca sobre confirmar o que já foi dito:',
    '  1. o VALOR, quando não estiver claro (nunca chute um número);',
    '  2. a FORMA DE PAGAMENTO de uma compra avulsa (crédito ou débito/Pix), quando a pessoa não',
    '     disser — a regra completa está na seção seguinte.',
    'Depois de lançar, confirme em uma frase curta o que foi feito (ex: "Pronto, lancei R$39,90',
    'da Netflix nas assinaturas 👍"), dizendo em qual lista entrou.',
    '',
    ...
    '- Se for uma compra pontual/avulsa (mercado, Uber, restaurante, roupa, farmácia), a FORMA DE',
    '  PAGAMENTO decide a ferramenta, e ela tem três caminhos:',
    '  a) Pix, débito, dinheiro, "na hora", "no dinheiro", "transferi", ou um comprovante de',
    '     Pix/transferência na imagem → adicionar_gasto_debito.',
    '  b) Crédito, cartão, "passei no cartão", "no crédito" → adicionar_compra_cartao.',
    '  c) A pessoa NÃO disse como pagou → PERGUNTE antes de lançar, numa frase curta e direta',
    '     ("Foi no crédito ou no débito/Pix?"). NÃO lance em nenhuma das duas listas enquanto ela',
    '     não responder, e NÃO chute pelo tipo da compra — mercado tanto pode ser crédito quanto',
    '     Pix, e cair na lista errada suja a fatura do cartão ou o orçamento do débito. Assim que',
    '     ela responder, lance na hora, sem pedir mais nenhuma confirmação.',
    '  Se ela mandar vários gastos de uma vez e disser a forma de pagamento de só alguns, lance os',
    '  que dá e pergunte de uma vez só pelos que ficaram sem.',
    ...
    context.categorias
      ? `Categorias das compras avulsas — valem para crédito e débito (id=rótulo): ${context.categorias}.`
      : '',
    '',
