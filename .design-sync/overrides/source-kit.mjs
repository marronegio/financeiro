// forked from design-sync lib/source-kit.mjs — synth entry must re-export default
// exports (`export { default as <Name> }`): every DinPrev component is a default
// export, and `export * from` alone leaves them off window.DinPrev.
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // Fork: also re-export each file's default under its PascalCase basename —
    // `export * from` skips defaults, and this repo's components are all
    // default exports. Skip when the file has no default or already exports a
    // same-named binding (would collide in esbuild).
    const lines = comps.map((p) => {
      ...
      const namedSame = new RegExp(`(^|\\n)\\s*export\\s+(const|function|class|let|var)\\s+${base}\\b`).test(text);
      if (hasDefault && !namedSame && /^[A-Z][A-Za-z0-9]*$/.test(base)) {
        out.push(`export { default as ${base} } from ${JSON.stringify(p)};`);
      }
      return out.join('\n');
    });
